<template>
  ...
  <div class="Mirle21Alarm">
    <div class="mt-2">
      <h4 class="fw-bold">K21盟立異常紀錄</h4>
      <div class="d-flex align-items-center mb-3">
        ...
        <button class="btn btn-outline-info btn-sm me-2" type="button" data-bs-toggle="collapse" data-bs-target="#advancedSearch" @click="searchData.alarmCode = ''">
          進階查詢
        </button>
      </div>
        ...
        <div class="row">
          <div class="col-2">
            <div class="input-group input-group-sm mb-3">
              <span class="input-group-text" id="basic-addon1">alarmCode</span>
              <input type="text" class="form-control" aria-label="alarmCode" v-model="searchData.alarmCode">
            </div>
          </div>
        </div>
      </div>
      <div class="d-flex align-items-center mb-3">
        ...
        <button class="btn btn-success btn-sm me-2" @click="alarmAdd">
          異常新增
        </button>
...
export default {
  ...
  methods: {
    search() {
      ...
      this.isLoading = true;
      const api = `${process.env.VUE_APP_CIM_ESERVICES_MIRLE21_ALARM_API_URL}/mirle21_alarm?startTime=${this.startTime}&endTime=${this.endTime}`;
      this.$http
        .get(api)
        .then((res) => {
          if (res.data.success) {
            ...
            res.data.data.rows.forEach((item) => {
              if (item[5].includes('Charger')) {
                item[0] = 'Charger';
                const st = item[5].indexOf(']');
                item[1] = item[5].substring(8, st) < 10 ? `0${item[5].substring(8, st)}` : item[5].substring(8, st);
              }
              const machineTemp = item[0];
              const machineNumberTemp = item[1];
              // const locationIDTemp = item[2];
              // const alarmTypeTemp = item[3];
              const alarmCodeTemp = item[4] !== '' ? item[4] : 'NA';
              const alarmDescTemp = item[5];
              const sTimeTemp = item[6];
              const eTimeTemp = item[7];
              const yearTemp = sTimeTemp.substring(0, 4);
              const dateTemp = sTimeTemp.substring(4, 8);
              const carrierIDTemp = item[8];
              //
            ...
            console.log('get mirle21_alarm success');
          } else {
            this.strModal = '查詢失敗！';
            this.boxModal.show();
            this.isLoading = false;
            console.log('get mirle21_alarm error');
          }
        })
        .catch((error) => {
          this.strModal = '查詢失敗！';
    ...
    searchAlarmCode(alarmCode, index) {
      this.isLoading = true;
      const api = `${process.env.VUE_APP_CIM_ESERVICES_MIRLE21_ALARM_API_URL}/mirle21_alarm_alarmcode?alarmCode=${alarmCode}`;
      this.$http
        .get(api)
        .then((res) => {
          if (res.data.success) {
            ...
            if (this.alarmCodeData !== undefined) {
              ...
              this.isLoading = false;
              console.log('get mirle21_alarm_alarmcode success');
            } else {
              this.isLoading = false;
            }
          } else {
            this.isLoading = false;
            console.log('get mirle21_alarm_alarmcode error');
          }
        })
        .catch((error) => {
          this.isLoading = false;
    ...
    searchAdded(startTimeString, index) {
      this.isLoading = true;
      const api = `${process.env.VUE_APP_CIM_ESERVICES_MIRLE21_ALARM_API_URL}/mirle21_alarm_startTimeString?startTimeString=${startTimeString}`;
      this.$http
        .get(api)
        .then((res) => {
          if (res.data.success) {
            ...
            if (temp !== undefined) {
              ...
              this.isLoading = false;
              console.log('get mirle21_alarm_startTimeString success');
            } else {
              this.isLoading = false;
            }
          } else {
            this.isLoading = false;
            console.log('get mirle21_alarm_startTimeString error');
          }
        })
        .catch((error) => {
          this.isLoading = false;
    ...
    historySearch(str) {
      ...
      this.isLoading = true;
      const api = `${process.env.VUE_APP_CIM_ESERVICES_MIRLE21_ALARM_API_URL}/mirle21_alarm_data?startTime=${this.historyStartTime}&endTime=${this.historyEndTime}&alarmCode=${this.searchData.alarmCode}`;
      this.$http
        .get(api)
        .then((res) => {
          if (res.data.success) {
            ...
            this.boxModal.show();
            this.isLoading = false;
            console.log('get mirle21_alarm_data success');
          } else {
            this.strModal = '查無此時間異常！';
            this.boxModal.show();
            this.isLoading = false;
            console.log('get mirle21_alarm_data error');
          }
        })
        .catch((error) => {
    ...
    historyRevise() {
      ...
      if (checkEmpty) {
        ...
        this.isLoading = true;
        const api = `${process.env.VUE_APP_CIM_ESERVICES_MIRLE21_ALARM_API_URL}/mirle21_alarm_data`;
        this.$http
          .patch(api, newData)
          .then((res) => {
            if (res.data.success) {
              ...
              this.isLoading = false;
              console.log('update mirle21_alarm_data success');
            } else {
              this.strModal = '修改失敗！';
              this.boxModal.show();
              this.isLoading = false;
              console.log('update mirle21_alarm_data error');
            }
          })
          .catch((error) => {
            this.strModal = '修改失敗！';
    ...
    historyDelete() {
      ...
      if (checkEmpty) {
        ...
        this.isLoading = true;
        const api = `${process.env.VUE_APP_CIM_ESERVICES_MIRLE21_ALARM_API_URL}/mirle21_alarm_data_delete`;
        this.$http
          .post(api, newData)
          .then((res) => {
            if (res.data.success) {
              ...
              this.isLoading = false;
              console.log('delete mirle21_alarm_data success');
            } else {
              this.strModal = '刪除失敗！';
              this.boxModal.show();
              this.isLoading = false;
              console.log('delete mirle21_alarm_data error');
            }
          })
          .catch((error) => {
            this.strModal = '刪除失敗！';
      ...
      if (checkEmpty) {
        ...
        this.$http.post(api, newData).then((res) => {
          if (res.data.success) {
            ...
            this.data = [];
            this.alarmCode = '';
            console.log('post mirle21_alarm_data success');
          } else {
            this.strModal = '新增失敗！';
            this.boxModal.show();
            this.isLoading = false;
            console.log('post mirle21_alarm_data error');
          }
        });
      }
    },
