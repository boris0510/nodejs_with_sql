<template>
  ...
  <div class="GPM22Alarm">
    <div class="mt-2">
      <h4 class="fw-bold">K22GPM異常紀錄</h4>
...
export default {
  ...
  methods: {
    search() {
      ...
      this.isLoading = true;
      const api = `${process.env.VUE_APP_CIM_ESERVICES_GPM22_ALARM_API_URL}/gpm22_alarm?startTime=${this.startTime}&endTime=${this.endTime}`;
      this.$http
        .get(api)
        .then((res) => {
          if (res.data.success) {
    ...
    searchAlarmCode(alarmCode, index) {
      this.isLoading = true;
      const api = `${process.env.VUE_APP_CIM_ESERVICES_GPM22_ALARM_API_URL}/gpm22_alarm_alarmcode?alarmCode=${alarmCode}`;
      this.$http
        .get(api)
        .then((res) => {
          if (res.data.success) {
            ...
            if (this.alarmCodeData !== undefined) {
              ...
              this.isLoading = false;
              console.log('get gpm22_alarm_alarmcode success');
            } else {
              this.isLoading = false;
            }
          } else {
            this.isLoading = false;
            console.log('get gpm22_alarm_alarmcode error');
          }
        })
        .catch((error) => {
          this.isLoading = false;
    ...
    searchAdded(startTimeString, index) {
      this.isLoading = true;
      const api = `${process.env.VUE_APP_CIM_ESERVICES_GPM22_ALARM_API_URL}/gpm22_alarm_startTimeString?startTimeString=${startTimeString}`;
      this.$http
        .get(api)
        .then((res) => {
          if (res.data.success) {
            ...
            if (temp !== undefined) {
              ...
              this.isLoading = false;
              console.log('get gpm22_alarm_startTimeString success');
            } else {
              this.isLoading = false;
            }
          } else {
            this.isLoading = false;
            console.log('get gpm22_alarm_startTimeString error');
          }
        })
        .catch((error) => {
          this.isLoading = false;
    ...
    historySearch(str) {
      ...
      this.isLoading = true;
      const api = `${process.env.VUE_APP_CIM_ESERVICES_GPM22_ALARM_API_URL}/gpm22_alarm_data?startTime=${this.historyStartTime}&endTime=${this.historyEndTime}`;
      this.$http
        .get(api)
        .then((res) => {
          if (res.data.success) {
            ...
            this.boxModal.show();
            this.isLoading = false;
            console.log('get gpm22_alarm_data success');
          } else {
    ...
    historyRevise() {
      ...
      if (checkEmpty) {
        ...
        this.isLoading = true;
        const api = `${process.env.VUE_APP_CIM_ESERVICES_GPM22_ALARM_API_URL}/gpm22_alarm_data`;
        this.$http
          .patch(api, newData)
          .then((res) => {
            if (res.data.success) {
              ...
              this.isLoading = false;
              console.log('update gpm22_alarm_data success');
            } else {
              this.strModal = '修改失敗！';
              this.boxModal.show();
              this.isLoading = false;
              console.log('update gpm22_alarm_data error');
            }
          })
          .catch((error) => {
            this.strModal = '修改失敗！';
    ...
    historyDelete() {
      ...
      if (checkEmpty) {
        ...
        this.isLoading = true;
        const api = `${process.env.VUE_APP_CIM_ESERVICES_GPM22_ALARM_API_URL}/gpm22_alarm_data_delete`;
        this.$http
          .post(api, newData)
          .then((res) => {
            if (res.data.success) {
              ...
              this.isLoading = false;
              console.log('delete gpm22_alarm_data success');
            } else {
              this.strModal = '刪除失敗！';
              this.boxModal.show();
              this.isLoading = false;
              console.log('delete gpm22_alarm_data error');
            }
          })
          .catch((error) => {
            this.strModal = '刪除失敗！';
      ...
      if (checkEmpty) {
        ...
        this.$http.post(api, newData).then((res) => {
          if (res.data.success) {
            ...
            this.alarmCodeData = [];
            this.alarmCode = '';
            console.log('post gpm22_alarm_data success');
          } else {
            this.strModal = '新增失敗！';
            this.boxModal.show();
            this.isLoading = false;
            console.log('post gpm22_alarm_data error');
          }
        });
      }
    },
